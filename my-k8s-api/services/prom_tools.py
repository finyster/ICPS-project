...
def list_namespace_names() -> str:
    """
    回傳所有 namespace 名稱
    """
    print("=== [DEBUG] list_namespace_names 被呼叫了 ===")
    try:
        from kubernetes import client, config
        config.load_kube_config()
        v1 = client.CoreV1Api()
...
def list_pod_names(namespace: str) -> str:
    """
    回傳指定 namespace 內所有 pod 名稱
    """
    try:
        from kubernetes import client, config
        config.load_kube_config()
        v1 = client.CoreV1Api()
        pods = [item.metadata.name for item in v1.list_namespaced_pod(namespace=namespace).items]
        return json.dumps({"namespace": namespace, "pods": pods})
    except Exception as e:
        return json.dumps({"error": str(e)})
###以上三個函數是alan52254新增的，列出所有 node, namespace, pod 名稱
